...
const UseCaseCard: React.FC<{ icon: React.ReactNode; title: string; desc: string; index: number }> = ({ icon, title, desc, index }) => (
  ...
  >
    <div className="w-12 h-12 bg-stone-100 rounded-[2px] flex items-center justify-center text-slate-900 group-hover:bg-[#B85C38] group-hover:text-white transition-colors mb-6 duration-300">
      {icon}
    </div>
...
export const UseCases: React.FC = () => {
  ...
  return (
    <section ref={ref} className="py-24 px-6 bg-white border-y border-slate-100 overflow-hidden">
